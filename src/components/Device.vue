    ...
    <v-row>
      <v-col v-for="device in devices" :key="device.id_device" cols="12" md="4">
        <v-card class="mt-4 pa-4">
          <v-row align="center">
            <v-col cols="8">
              <span class="text-subtitle-1">{{ device.name_device }}</span>
              <p class="text-caption">{{ device.room }}</p>
            </v-col>
            <v-col cols="4" class="text-right">
              <span class="text-h6 font-weight-bold" :class="device.power_watts < 0 ? 'red--text' : 'green--text'">
